Param(
    ...
    [string]$SonarQubeProjectKey = "OpenResultCSharp",
...
function RunTestsAndGenerateCoverageReport {
	try {
		...
		reportgenerator "-reports:OpenResult.Tests\TestResults\**\coverage.cobertura.xml" "-targetdir:sonarqubecoverage" "-reporttypes:SonarQube"
	}
	catch {
...
function CleanUpTestResultsDirectory {
	try {
		Remove-Item .\OpenResult.Tests\TestResults\ -Force -Recurse -ErrorAction Stop
	}
	catch {
